...
from config import API_CONFIG, PROJECT_NAME, ROOT_DIR
...
def post(headers, address, mime_type, timeout=10, data=None, files=None, cookies=None):
    ...
    if not mime_type:
        mime_type = ""
    if 'multipart/form-data; boundary=file' in mime_type:
        for key, value in files.items():
            # value = files[key]
            # 判定参数值是否为文件，如果是则替换为二进制值
            if '/' in value and os.path.isfile(ROOT_DIR + value):
                file_path = ROOT_DIR + value
                files[key] = (os.path.basename(value), open(file_path, 'rb'))
