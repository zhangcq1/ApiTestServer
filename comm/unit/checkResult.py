...
from comm.unit import readRelevance, replaceRelevance, dealCache


def check_jsonpath(src_data, dst_data):
    for key, value in src_data.items():
        all_values = dealCache.jsonpath_find(dst_data, key)
        if not all_values:
            if value is not None:
                raise ValueError("结果校验失败，{}的期望值应为{},返回结果不存在该参数值".format(key, value))
        elif str(value) not in str(all_values):
            raise ValueError("结果校验失败，{}的期望值应为{},返回结果为{}".format(key, value, all_values[0]))
    ...
    if isinstance(src_data, dict):
        for key in src_data:
            if key not in dst_data:
                ...
                this_key = key
                if isinstance(src_data[this_key], str):
                    if "type:" in src_data[this_key]:
                        type_map = {
                            "integer": int,
                            "float": float,
                            "string": str,
                        ...
                        if type(dst_data[this_key]) == type_map.get(src_data[this_key].split('type:')[-1]):
                            continue
                        else:
                            raise Exception("JSON格式校验，关键字 %s 返回结果 %s 与期望结果 %s 类型不符"
                                            % (this_key, src_data[this_key], dst_data[this_key]))
                    if "value:" in src_data[this_key]:
                        src_value = str(src_data[this_key].split('value:')[-1].strip())
                        if isinstance(dst_data[this_key], str):
                            dst_value = dst_data[this_key]
                        else:
                            dst_value = str(dst_data[this_key])
                        if src_value == dst_value:
                            continue
                        else:
                            raise Exception("值校验错误，关键字 %s 返回结果 %s 与期望结果 %s 类型不符"
                                            % (this_key, src_data[this_key], dst_data[this_key]))
                if isinstance(src_data[this_key], dict) and isinstance(dst_data[this_key], dict):
                    check_json(src_data[this_key], dst_data[this_key])
                elif not src_data[this_key]:
                    pass
...
def check_result(case_data, code, data):
    ...
    if check_type == 'no_check':
    ...
    elif check_type == 'check_jsonpath':
        with allure.step("jsonpath方式校验返回值"):
            allure.attach(name="实际code", body=str(code))
            allure.attach(name="期望code", body=str(expected_code))
            allure.attach(name='实际data', body=str(data))
            allure.attach(name='期望data', body=str(expected_result))
        if int(code) == expected_code:
            if not data:
                data = "{}"
            check_jsonpath(expected_result, data)
        else:
            raise Exception("接口状态码错误！\n %s != %s" % (code, expected_code))
